...
Do **not** expand lua to `max_fps` / palette fonts / live reload / `default_prog` / `augment-command-palette` / hyperlink hover highlight unless asked. `enable_scroll_bar` is **050**. `launch_menu` is in the 049 ShowLauncher Picker. WSL/`wsl_domains` is **053**. Workspaces are **055**. Unix Attach stays out.
...
1. **049** user-try if not done: remaining palette catalog (charselect, copy mode, launcher, reload). 050 overlays already user-ok.
...
If the user just says “continue”: wait for a pick among leftover try/polish (049 catalog, 034 hover); do **not** start 026, unix Attach, `Mux::split_pane` rewrite, or hyperlink hover.
