...
| Palette AdjustPaneSize (048) | Wired. Resize pane 1 cell L/R/U/D via `ResizableState`. **Needs user-try.** |
| Palette catalog complete (049) | Wired. All 89 `PALETTE_COMMANDS` rows run. Shared `Picker` + second window + copy mode. **Needs user-try.** |
| Overlay parity + scrollbar (050) | Wired. Vim search (Ctrl+Shift+F), quick-select labels, pane letter badges, New Window offset, ActivateWindow raise, lua scrollbar, FPS palette-only. Debug lua REPL backlog. **Needs user-try.** |
| New-window cascade / search copy (051) | Wired. Cascade New Window off occupied HWND; focus new HWND; larger centered pane badge. Search copy retried in **052**. **Needs user-try.** |
| Close this window / search copy (052) | Wired. Last tab/`exit` of one HWND does not quit the others; Ctrl+Shift+C / palette Copy keep the search hit. **Needs user-try.** |
| Mux WSL/exec domains (053) | Wired. `wsl -l -v` / lua `wsl_domains` + `exec_domains` as `LocalDomain`. Chevron / launcher / palette New Tab (Domain …). Plus still cmd.exe. **Needs user-try.** |
| Tab sends PTY completion (054) | Wired. AppShell `tab` / `shift-tab` beat gpui-component Root `focus_next`. **Needs user-try.** |
...
`go_nogo`: in-process embed = **no**. Continue POC = **yes**. Runtime window = **yes**. CLI spawn = **yes**. Min usable chrome = **yes**. Mux cmd.exe = **user-tried**. Paint = **line sprites user-ok (017)**; **025 4K 120dpi user-ok**. FPS HUD = **palette-only (050)**. Lua config = **user-ok (020; Cascadia + Dracula)**; **034 user-ok** except hover highlight; **050 `enable_scroll_bar` wired**. Selection/copy/paste = **user-ok (021, including wrap)**. 022 click/focus = **user-ok**. 023 box-draw = **user-ok** (tight clip reverted). 026 monitor-move hang = **backlog**. **027 shell menu = user-ok.** **028 icons = user-ok.** **029 cursor gap = not user-ok.** **030 integer cell grid = user-ok.** **031 cursor until backspace = backlog.** **032 tab close/exit keys = better now.** **033 last-tab `exit` quits = user-ok.** **037 dialog cancel focus = keep after dismiss.** **038 dialog layer mounted = user-ok.** **039 skip close confirm for skip-listed shells = needs user-try.** **040 splits = user-ok.** **041 inactive pane HSB = user-ok.** **042 palette selection invert = user-ok.** **043 palette ↑↓ keep selection = user-ok.** **044 palette core/tab chrome = user-ok (41 wired).** **045 palette ↑↓ scroll-into-view = user-ok.** **046 palette pane dir/rotate/zoom = user-ok.** **047 palette window chrome = user-ok.** **048 palette AdjustPaneSize = wired, needs user-try.** **049 palette catalog complete = 89 wired / 0 listed, needs user-try.** **050 overlay parity + scrollbar = wired, needs user-try.** **051 new-window cascade = wired, needs user-try.** **052 close-this-window + search copy = wired, needs user-try.** **053 WSL/exec mux domains = wired, needs user-try.** **054 Tab → PTY completion = wired, needs user-try.** Glyph atlas Element = **not started**. Scrollback = **user-ok**. Palette = **036 listing + 049 all Wired + 050 ToggleFpsHud + 053 domain rows**. ConPTY junk = **shared with wezterm-gui, polish later**.
