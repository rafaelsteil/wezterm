...
| Mux workspaces (055) | Wired. HWND ↔ MuxWindow; new tabs `Domain::spawn`; create/switch hide/show HWNDs (keep `SplitLayout`). Last HWND of a workspace falls back if another remains. **Needs user-try.** |
...
5. Come back later: unix Attach; debug overlay **lua REPL**; 049 Picker path (`find::PICKER_SEARCH_QUICKSELECT_PANESELECT`); FPS HUD FRAME-for-typing mode.
6. Come back later: monitor-move hang (026) — 2–3s when dragging between 96dpi and 120dpi monitors.
7. Come back later: cursor missing until backspace (031) — launch/new tab; space hides it.
8. GPUI `text_system` spike only if they ask for more speed (`docs/reference/gpui-text-vs-sprites.md`).
9. **Only if asked:** windowing cutover.

If the user just says “continue”: wait for the 055 workspace try; do **not** start 026, 031, unix Attach, `Mux::split_pane` rewrite, or hyperlink hover.
...
- `docs/plans/055-mux-workspaces.md` — HWND ↔ MuxWindow; hide/show HWNDs; Domain::spawn tabs (needs user-try)
