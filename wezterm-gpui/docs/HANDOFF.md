...
| Tab title from dropdown (057) | Wired. Tab chrome uses chevron `profile.label` (Command Prompt, WSL:…), not `cmd.exe`. Rename tab override stays. **Needs user-try.** |
...
If the user just says “continue”: wait for a pick among leftover try/polish (057 tab titles, 049 catalog, 034 hover); do **not** start 026, unix Attach, `Mux::split_pane` rewrite, or hyperlink hover.
...
wezterm-gpui/                    # OWN Cargo workspace (excluded from root)
  ...
  src/term_pane.rs               # mux LocalPane; wezterm-font paint; 021 selection; 056 search viewport; 057 tab title = profile.label
...
45. **Tab title is the dropdown label (057).** `LocalPane::get_title` turns default `"wezterm"` into `cmd.exe` / `wsl.exe`. Tab chrome uses `profile.label` (Command Prompt, WSL:Ubuntu, …). Palette Rename tab still sets `title_override`.
