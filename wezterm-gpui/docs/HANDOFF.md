...
Dual-stack is expected. POC shortcuts are allowed. Palette **listing** is in (036); **049** wired every static catalog row (no dimmed rows). Overlays use a shared `Picker` (not termwiz).
...
| Geometry box-draw + per-cell clip (023) | **Finished.** Box-draw **user-ok** 2026-08-27 (`echo` / `tree`). Tight cell clip **reverted (025)** (intentional). Powerline is not 023. |
...
| 120dpi cursor gap (029) | **User-ok** 2026-08-28. Gap closed after 030 integer cell grid. `cell_span` kept as fill helper. |
| Integer cell grid (030) | **User-ok** 2026-08-27 (“works great now”). Ceil cell size + glyphs step `num_cells * cell_w`. |
| Cursor missing until backspace (031) | **User-ok** 2026-08-28 (“fixed”). Fill `cursor_bg` at `cursor.x` even when `visible_cells()` has no matching col. |
| Tab close / `exit` kills typing (032) | **Better now.** Close tab on pane death; restore AppShell focus. |
| Last tab `exit` quits (033) | **User-ok** 2026-08-27 (“works great”). Last-tab process exit quits the last HWND. **052:** other HWNDs stay. |
...
| Skip close confirm for shells (039) | **User-ok** 2026-08-28. cmd.exe / skip list does not prompt; AlertDialog kept for stateful panes. |
...
| Palette AdjustPaneSize (048) | **User-ok** 2026-08-28. Resize pane 1 cell L/R/U/D via `ResizableState`. |
| Palette catalog complete (049) | Wired. All 89 `PALETTE_COMMANDS` rows run. Shared `Picker` + second window + copy mode. **Needs user-try.** |
| Overlay parity + scrollbar (050) | **User-ok** 2026-08-28. Vim search (Ctrl+Shift+F), quick-select labels, pane letter badges, New Window offset, ActivateWindow raise, lua scrollbar, FPS palette-only. Debug lua REPL backlog. |
| New-window cascade / search copy (051) | **User-ok** 2026-08-28. Cascade New Window off occupied HWND; focus new HWND; larger centered pane badge. Search copy retried in **052**. |
| Close this window / search copy (052) | **User-ok** 2026-08-28. Last tab/`exit` of one HWND does not quit the others; Ctrl+Shift+C / palette Copy keep the search hit. |
| Mux WSL/exec domains (053) | **User-ok** 2026-08-28. `wsl -l -v` / lua `wsl_domains` + `exec_domains` as `LocalDomain`. Chevron / launcher / palette New Tab (Domain …). Plus still cmd.exe. |
| Tab sends PTY completion (054) | **User-ok** 2026-08-28. AppShell `tab` / `shift-tab` beat gpui-component Root `focus_next`. |
| Mux workspaces (055) | **User-ok** 2026-08-28. HWND ↔ MuxWindow; new tabs `Domain::spawn`; create/switch hide/show HWNDs (keep `SplitLayout`). Last HWND of a workspace falls back if another remains. OS X matches `exit`. |
| Search overlay no PTY shrink (056) | **User-ok** 2026-08-29 (“Fixed”). Search bar overlays the last terminal row (no flex shrink / ConPTY). Newest-visible hit; restore viewport on close. |
| Tab title from dropdown (057) | **User-ok** 2026-08-29. Tab chrome uses chevron `profile.label` (Command Prompt, WSL:…), not `cmd.exe`. Rename tab override stays. |
| Hyperlink hover (058) | **User-ok** 2026-08-29. Overlay underline + PointingHand on hovered URL. |
| Plain click opens link (059) | **User-ok** 2026-08-29. Unmodified left-up with no selection opens the hovered URL when defaults are on. |
| Honor `disable_default_mouse_bindings` (060) | **User-ok** 2026-08-29. With the flag, plain click does not open; Ctrl+click still opens. Hover still underlines. |
| Honor `command_palette_font` (061) | **User-ok** 2026-08-29. Lua Segoe UI + 12pt on the Command Palette overlay. |
| Honor `window_decorations` (062) | **User-ok** 2026-08-29. Native `TITLE` caption; `INTEGRATED_BUTTONS` one chrome row; tab-bar drag including `NONE`. |
| Lua config second slice (034) | **User-ok** 2026-08-27 (tab chrome + Ctrl+click open + Ctrl+wheel). **058** hover underline + hand cursor **user-ok**. **059** plain click **user-ok**. **060** disable_default_mouse_bindings **user-ok**. **061** palette font **user-ok**. **062** window_decorations **user-ok**. Fancy tab bar: no visible change. `adjust_window_size` not tested. Matrix: `docs/lua-config.json`. |
| Command palette + launcher matrix (036) | Tracking JSON + full Windows default list in the GPUI palette. **049: all 89 catalog rows Wired** (no dimmed). Launcher is a `Picker`, not a termwiz clone. Matrices: `docs/command-palette.json`, `docs/launcher.json`. |

`go_nogo`: in-process embed = **no**. Continue POC = **yes**. Runtime window = **yes**. CLI spawn = **yes**. Min usable chrome = **yes**. Mux cmd.exe = **user-tried**. Paint = **line sprites user-ok (017)**; **025 4K 120dpi user-ok**. FPS HUD = **palette-only (050)**. Lua config = **user-ok (020; Cascadia + Dracula)**; **034 user-ok**; **058 hover user-ok**; **059 plain click user-ok**; **060 disable_default_mouse_bindings user-ok**; **061 command_palette_font user-ok**; **062 window_decorations user-ok**; **050 `enable_scroll_bar` user-ok**. Selection/copy/paste = **user-ok (021, including wrap)**. 022 click/focus = **user-ok**. 023 box-draw = **finished** (tight clip reverted 025). 026 monitor-move hang = **backlog**. **027 shell menu = user-ok.** **028 icons = user-ok.** **029 cursor gap = user-ok.** **030 integer cell grid = user-ok.** **031 cursor at cursor.x = user-ok.** **032 tab close/exit keys = better now.** **033 last-tab `exit` quits = user-ok.** **037 dialog cancel focus = user-ok.** **038 dialog layer mounted = user-ok.** **039 skip close confirm for skip-listed shells = user-ok.** **040 splits = user-ok.** **041 inactive pane HSB = user-ok.** **042 palette selection invert = user-ok.** **043 palette ↑↓ keep selection = user-ok.** **044 palette core/tab chrome = user-ok (41 wired).** **045 palette ↑↓ scroll-into-view = user-ok.** **046 palette pane dir/rotate/zoom = user-ok.** **047 palette window chrome = user-ok.** **048 palette AdjustPaneSize = user-ok.** **049 palette catalog complete = 89 wired / 0 listed, needs user-try.** **050 overlay parity + scrollbar = user-ok.** **051 new-window cascade = user-ok.** **052 close-this-window + search copy = user-ok.** **053 WSL/exec mux domains = user-ok.** **054 Tab → PTY completion = user-ok.** **055 mux workspaces = user-ok.** **056 search overlay no PTY shrink = user-ok.** **057 tab title from dropdown = user-ok.** **058 hyperlink hover = wired, needs user-try.** Glyph atlas Element = **not started**. Scrollback = **user-ok**. Palette = **036 listing + 049 all Wired + 050 ToggleFpsHud + 053 domain rows + 055 workspace rows**. ConPTY junk = **shared with wezterm-gui, polish later**.
...
Default `wezterm gpui` hosts **cmd.exe** through mux (Plus / Ctrl+T). Chevron next to Plus lists Command Prompt, Windows PowerShell, and `pwsh` if installed (027) — **user-ok**. Icons need `gpui-component-assets` (028) — **user-ok**. Paint is **one cached GPUI image per line** (wezterm-font composite, 017) at window DPI — **user-ok** (“much better”). Live drag rewraps display only; ~450ms later one ConPTY `resize`. Loads **`~/.wezterm.lua`** for font/size/scheme/scrollback/bell (020) — **user-ok** (“works like a charm”). **034** tab chrome + that file’s `mouse_bindings` — **user-ok**; **058** hover underline **user-ok**; **059** plain click **user-ok**; **060** `disable_default_mouse_bindings` **user-ok**. Per-key matrix: **`docs/lua-config.json`**. Mouse **selection + copy/paste** (021) **user-ok** including wrapped triple-click. **023 finished:** box-draw (`echo` / `tree`); tight cell clip **reverted (025)**. **025 user-ok:** 4K 120dpi glyphs. **029 user-ok** (`cell_span`; gap closed after 030). **030 user-ok:** integer cell grid (“works great now”). **026 backlog:** 2–3s hang when moving the window between monitors. **031 user-ok:** fill cursor at `cursor.x` even past last stored cell (“fixed”). **032:** tab X / `exit` left typing dead — **better now**. **033 user-ok:** last-tab `exit` quits the last HWND (“works great”); **052** keeps other windows. **037 user-ok:** Ctrl+Q / tab X Cancel restores typing. **038:** Ctrl+Q showed **no dialog** — AppShell never painted `Root::render_dialog_layer` — **user-ok**. **039:** close confirm is not default wezterm — skip `cmd.exe` / skip list; keep AlertDialog. **User-ok.** **040 user-ok:** tab/shell splits via palette Split H/V (“works great”). **041 user-ok:** inactive pane HSB + hollow cursor (“perfect”). **042 user-ok:** palette selected row inverts lua fg/bg (“Confirmed colors work”). **043 user-ok:** ↑↓ in the palette keep the highlight (“All good”). **044 user-ok:** more palette rows run (scroll, reset, tabs, Help, minimize) (“all of these work”). **045 user-ok:** ↑↓ in the palette scroll the list into view (“all good now”). **046 user-ok:** palette Activate Pane / Rotate / Toggle Zoom on splits (“works like a charm”). **047 user-ok:** Toggle full screen / Always on Top / Reset font+window size (“everything works”). **048 user-ok:** Resize pane 1 cell L/R/U/D. **049:** remaining palette catalog wired in one shot (SpawnWindow, launcher, charselect, copy mode, search, pane select, reload). **Needs user-try.** **050 user-ok:** search/quickselect/paneselect match wezterm-gui; New Window offset; ActivateWindow raise; lua scrollbar; FPS palette-only. **052 user-ok:** last tab/`exit` of one HWND must not kill the others; Copy during Search. **053 user-ok:** WSL distros on the chevron / launcher / palette. **054 user-ok:** Tab is shell completion, not chrome focus. **055 user-ok:** Create/switch workspace (hide/show HWNDs; splits stay). **056 user-ok:** vim Ctrl+Shift+F no longer shrinks the PTY or jumps to scrollback (“Fixed”). **057 user-ok:** tab titles match the chevron. **058 user-ok:** hover underline + hand cursor. **059 user-ok:** plain click opens. **060 user-ok:** `disable_default_mouse_bindings`. **061:** `command_palette_font` **user-ok.** **062:** `window_decorations` **user-ok.** **Do not start** 026 unless asked.

Do **not** start a `window/` cutover unless the user explicitly asks.
Palette **listing** is 036; **049 wired all 89 catalog rows** (no dimmed). **050** restyles Search / Quick Select / Pane Select to wezterm-gui (049 Picker behind a flag). Split H/V **wired (040)**. Copy mode is still simplified hjkl. Charselect / launcher stay `Picker`.
...
Do **not** expand lua to `max_fps` / live reload / `default_prog` / `augment-command-palette` unless asked. `command_palette_font` is **061**. `window_decorations` is **062** (**user-ok**). `enable_scroll_bar` is **050**. `launch_menu` is in the 049 ShowLauncher Picker. WSL/`wsl_domains` is **053**. Workspaces are **055**. Unix Attach stays out. Vim search PTY shrink is **056**. Tab titles from the dropdown are **057** (**user-ok**). Hyperlink hover is **058** (**user-ok**). Plain click open is **059** (**user-ok**). `disable_default_mouse_bindings` is **060** (**user-ok**).

Workstream: `docs/reference/rendering-quality.md`. Lua matrix: `docs/lua-config.json` + `docs/reference/lua-config.md`. Palette matrix: `docs/command-palette.json` + `docs/reference/command-palette.md`. Launcher matrix: `docs/launcher.json` + `docs/reference/launcher.md`. Lua slices: `docs/plans/020-lua-config-first-slice.md` + `docs/plans/034-lua-config-second-slice.md`. Selection: `docs/plans/021-mouse-selection-copy-paste.md`. Box-draw: `docs/plans/023-boxdraw-and-cell-clip.md`. 4K dest: `docs/plans/024-hi-dpi-line-sprite-dest.md`. 120dpi clip/dest: `docs/plans/025-120dpi-glyph-clip-and-dest-1to1.md`. Monitor-move hang (parked): `docs/plans/026-monitor-move-dpi-reshape-hang.md`. New-tab menu: `docs/plans/027-new-tab-shell-menu.md`. Icons: `docs/plans/028-gpui-component-icon-assets.md`. Cursor gap (user-ok): `docs/plans/029-120dpi-cursor-cell-span.md`. Integer cell grid: `docs/plans/030-integer-cell-grid.md`. Cursor at cursor.x: `docs/plans/031-cursor-block-until-backspace.md`. Tab close/exit keys: `docs/plans/032-tab-close-exit-keys.md`. Last tab `exit` quits: `docs/plans/033-last-tab-exit-quits.md`. Palette+launcher matrix: `docs/plans/036-command-palette-and-launcher-matrix.md`. Dialog cancel/Ctrl+Q focus: `docs/plans/037-dialog-cancel-restores-term-focus.md`. Dialog layer mount: `docs/plans/038-mount-root-dialog-layer.md`. Skip close confirm: `docs/plans/039-skip-close-confirmation-for-shells.md`. Tab/shell splits: `docs/plans/040-tab-shell-splits.md`. Inactive pane HSB: `docs/plans/041-inactive-pane-hsb.md`. Palette selection contrast: `docs/plans/042-palette-selection-contrast.md`. Palette arrow keys: `docs/plans/043-palette-arrow-keeps-selection.md`. Palette core/tab chrome: `docs/plans/044-palette-core-chrome.md`. Palette arrow scroll: `docs/plans/045-palette-arrow-scrolls-list.md`. Palette pane ops: `docs/plans/046-palette-pane-ops.md`. Palette window chrome: `docs/plans/047-palette-window-chrome.md`. Palette AdjustPaneSize: `docs/plans/048-palette-adjust-pane-size.md`. Palette catalog complete: `docs/plans/049-palette-complete.md`. Overlay parity + scrollbar: `docs/plans/050-overlay-parity-and-scrollbar.md`. New-window cascade: `docs/plans/051-new-window-cascade-search-copy.md`. Close this window + search copy: `docs/plans/052-close-window-not-app-search-copy.md`. Mux WSL/exec domains: `docs/plans/053-mux-wsl-domains.md`. Tab PTY completion: `docs/plans/054-tab-sends-pty-completion.md`. Mux workspaces: `docs/plans/055-mux-workspaces.md`. Search overlay no PTY shrink: `docs/plans/056-search-overlay-no-pty-shrink.md`. Tab title from dropdown: `docs/plans/057-tab-title-from-dropdown.md`. Hyperlink hover: `docs/plans/058-hyperlink-hover-highlight.md`. Plain click opens link: `docs/plans/059-plain-click-opens-link.md`. Honor `disable_default_mouse_bindings`: `docs/plans/060-disable-default-mouse-bindings.md`. Honor `command_palette_font`: `docs/plans/061-command-palette-font.md`. Honor `window_decorations`: `docs/plans/062-window-decorations.md`.

Reasonable continuations, smallest first:

1. **049** user-try if not done: remaining palette catalog (charselect, copy mode, launcher, reload). 062 window_decorations **user-ok**. 061 palette font **user-ok**.
2. Come back later: unix Attach; debug overlay **lua REPL**; 049 Picker path (`find::PICKER_SEARCH_QUICKSELECT_PANESELECT`); FPS HUD FRAME-for-typing mode.
...
If the user just says “continue”: wait for a pick among leftover try/polish (049 catalog); do **not** start 026, unix Attach, or `Mux::split_pane` rewrite.
...
wezterm-gpui/                    # OWN Cargo workspace (excluded from root)
  ...
  src/shell.rs                   # TitleBar + tabs + Plus/chevron shell menu + mux TermPane + palette; 056 search overlay on term-host
  src/commands.rs                # Windows default palette catalog (036); 049 all Wired
  src/palette.rs                 # Input + filtered catalog; 042 selected invert; 043 Change-only reset; 045 ScrollHandle
  src/find.rs                    # 050 search hits / quick-select; 056 pick_current_hit newest-visible
  src/picker.rs                  # 049 searchable overlay (launcher, charselect, search, pane select, …)
  src/split_layout.rs            # GPUI pane tree (040); 046 dir/rotate/zoom; 048 ancestor_split; 049 extract/swap
  src/win_zorder.rs              # 047 Win32 HWND_TOPMOST / HWND_BOTTOM
  src/confirm.rs                 # AlertDialog confirm + Dialog+Input line prompt
  src/mux_host.rs                # config + Mux + LocalDomain; load lua; WSL/exec domains (053); spawn CommandBuilder
  src/lua_ui.rs                  # tab title / last-active helpers (034)
  src/shells.rs                  # cmd / Windows PowerShell / optional pwsh + mux domain profiles (053)
  src/hover.rs                   # 058 hyperlink Arc ptr_eq + underline spans
  src/term_pane.rs               # mux LocalPane; wezterm-font paint; 021 selection; 056 search viewport; 057 tab title = profile.label; 058 hover
  src/glyph_paint.rs             # wezterm-font → cached per-line RenderImages (017); selection tint; 025 dest 1:1; 029 cell_span; 030 integer grid; 031 cursor at cursor.x
  ...
  docs/
    ...
    plans/048-palette-adjust-pane-size.md
...
17. **Box-draw is CPU geometry in the line sprite, not GPUI `Path`.** wezterm-gui `customglyph.rs` is tiny-skia + the GPU atlas; do not path-dep `wezterm-gui`. tty7 `boxdraw.rs` is Apache — rewrite into pixels. Honor `custom_block_glyphs`. **023 finished.** Powerline (U+E0B0) is a later slice if nerd-font prompts look wrong.
...
23. **120dpi cursor 1px gap (029, user-ok after 030).** Cell bg/cursor used `round(col*cell_w)+round(cell_w)`. At 1.25 those snaps can skip a pixel. `cell_span` abuts fills but does **not** move the cursor’s left edge. 030 ceil + `num_cells * cell_w` closed the gap. User 2026-08-28: OK.
...
25. **Cursor missing until backspace (031, user-ok).** Block cursor hidden on launch/new tab; typing did not show it; backspace did; space hid it. Cursor fill walked `visible_cells()`; VT cursor often sits past the last stored cell. Fill `cursor_bg` at `cursor.x` like wezterm-gui. User 2026-08-28: “fixed.”
...
38. **046 pane ops are GPUI-tree, not mux Tab.** ActivatePaneDirection uses equal-half neighbor geometry. Rotate cycles leaf identities. Zoom paints only the active leaf. **User-ok** (“works like a charm”). AdjustPaneSize is 048.
...
40. **048 AdjustPaneSize is ResizableState, not mux Tab.** Keep one entity per split id. Mux walks up to the nearest matching axis and mutates the first child (± one cell). Zoomed is a no-op. gpui-component min size is ~100px.
41. **049 palette catalog is complete (POC shortcuts).** Shared `Picker` overlay for launcher/tab/pane/char/search/quickselect/debug. Second HWND via `cx.open_window`. Copy mode is hjkl/v/y on TermPane. Primary = clipboard. DetachDomain/ClearKeyTableStack notify. Search picker is visible lines; Ctrl+Shift+F remains FPS HUD. No WSL/unix/workspaces **in 049**. User asked to finish every dimmed row.
42. **053 WSL is LocalDomain, not wezterm-client.** `wsl -l -v` / lua `wsl_domains` → `LocalDomain::new_wsl` + `mux.add_domain`. Spawn with `cmd` `None` so `fixup_command` builds `wsl.exe --distribution`. Wrapping cmd.exe argv on a WSL domain would `--exec cmd.exe`. Chevron/launcher/palette list attached spawnable domains except `local`. Unix/SSH/TLS attach still need `wezterm-client` — do not path-dep `wezterm-mux-server-impl`. Plus / Ctrl+T stay Command Prompt.
43. **Tab is stolen by Root `focus_next` (054).** gpui-component Root binds `tab`/`shift-tab` in context `"Root"` before AppShell `on_key_down`. Bind those keys in `AppShell` context and send `KeyCode::Tab` to the PTY. `cx.propagate()` when a dialog is open so OK/Cancel still cycle. Palette already uses `PaletteOpen`.
44. **Search bar must not be a flex sibling (056).** A child between `term-host` and the status bar shrinks the pane → `resize_display` + 450ms ConPTY, so vim loses the first row for ~1s. Overlay `absolute().bottom_0()` on `term-host` like wezterm-gui’s last-row search UI. Typing used to jump to the oldest scrollback hit (`hits[0]` top-down); reverse newest-first and `pick_current_hit` visible. `clear_search` restores `viewport_on_open` or the live snap waits for a PTY key. **User-ok** (“Fixed”).
45. **Tab title is the dropdown label (057).** `LocalPane::get_title` turns default `"wezterm"` into `cmd.exe` / `wsl.exe`. Tab chrome uses `profile.label` (Command Prompt, WSL:Ubuntu, …). Palette Rename tab still sets `title_override`. **User-ok.**
46. **Hyperlink hover is an overlay, not a row-sprite (058).** wezterm-gui bakes underline into the glyph atlas (`current_highlight` + `cached_line_sprite`). GPUI keeps 017 line-sprite cache; hover paints a fg quad + `CursorStyle::PointingHand`. Match with `Arc::ptr_eq`. Hover is not Ctrl-gated. **User-ok.**
47. **Plain click opens the hovered link (059).** 034 only honored user lua Ctrl+click. wezterm-gui default InputMap still binds unmodified Left Up to `CompleteSelectionOrOpenLinkAtMouseCursor` unless `disable_default_mouse_bindings`. Empty selection → open `current_highlight`. Copy-on-select stays Ctrl+Shift+C. **User-ok** 2026-08-29.
48. **`disable_default_mouse_bindings` skips that default (060).** 059 applied open-on-click even when the flag was true. wezterm-gui then only highlights. Gate the default on the flag; user lua Ctrl+click still opens. Hover (058) stays. 021 selection stays hardcoded. **User-ok** 2026-08-29.
49. **`window_decorations` is not “TitleBar already has buttons” (062).** 034 marked it already_matched. Native `TITLE` uses an OS caption and no GPUI TitleBar. GPUI Windows never sets `WS_CAPTION`; apply it via `SetWindowLongPtr` after create. `INTEGRATED_BUTTONS` puts TabBar inside TitleBar (one row). `RESIZE`/`NONE` have no window buttons. Empty tab-bar space is always `WindowControlArea::Drag` (wezterm-gui `TabBarItem::None`), including `NONE`. Left alignment / custom button list / macOS flags parked.
...
- `docs/plans/023-boxdraw-and-cell-clip.md` — geometry U+2500–259F (**finished**; tight cell clip reverted in 025)
...
- `docs/plans/029-120dpi-cursor-cell-span.md` — abutting cell fills (**user-ok**; gap closed by 030)
- `docs/plans/030-integer-cell-grid.md` — ceil cell size + `num_cells * cell_w` glyphs (**user-ok**)
- `docs/plans/031-cursor-block-until-backspace.md` — fill cursor at `cursor.x` even past last stored cell (**user-ok**)
...
- `docs/plans/034-lua-config-second-slice.md` — remaining ~/.wezterm.lua chrome + mouse_bindings (**user-ok**; 058 hover **user-ok**; 059 plain click **user-ok**; 060 disable_default_mouse_bindings **user-ok**)
- `docs/plans/036-command-palette-and-launcher-matrix.md` — classify palette/launcher ops; dimmed NYI list
- `docs/plans/037-dialog-cancel-restores-term-focus.md` — Ctrl+Q / tab X Cancel left typing dead (032 leftover) (**user-ok**)
- `docs/plans/038-mount-root-dialog-layer.md` — AppShell must paint `Root::render_dialog_layer` (**user-ok**)
- `docs/plans/039-skip-close-confirmation-for-shells.md` — skip close confirm for cmd.exe / skip list (**user-ok**)
...
- `docs/plans/048-palette-adjust-pane-size.md` — Resize pane 1 cell L/R/U/D (`ResizableState`; **user-ok**)
- `docs/plans/049-palette-complete.md` — remaining catalog Wired (Picker + second window + copy mode; needs user-try)
- `docs/plans/050-overlay-parity-and-scrollbar.md` — vim search / quick-select / pane badges / scrollbar / window offset (**user-ok**)
- `docs/plans/051-new-window-cascade-search-copy.md` — cascade New Window, focus new HWND, badge, search copy (**user-ok**)
- `docs/plans/052-close-window-not-app-search-copy.md` — last tab/`exit` closes this HWND only; search copy (**user-ok**)
- `docs/plans/053-mux-wsl-domains.md` — WSL/exec LocalDomain registration; chevron/launcher/palette (**user-ok**)
- `docs/plans/054-tab-sends-pty-completion.md` — Tab/Shift+Tab to PTY, not Root focus_next (**user-ok**)
- `docs/plans/055-mux-workspaces.md` — HWND ↔ MuxWindow; hide/show HWNDs; Domain::spawn tabs (**user-ok**)
- `docs/plans/056-search-overlay-no-pty-shrink.md` — vim search bar must not shrink PTY / jump scrollback (**user-ok**)
- `docs/plans/057-tab-title-from-dropdown.md` — tab chrome uses chevron `profile.label` (**user-ok**)
- `docs/plans/058-hyperlink-hover-highlight.md` — hover underline + PointingHand (**user-ok**)
- `docs/plans/059-plain-click-opens-link.md` — plain click opens hovered URL (**user-ok**)
- `docs/plans/060-disable-default-mouse-bindings.md` — honor `disable_default_mouse_bindings` for plain-click open (**user-ok**)
- `docs/plans/061-command-palette-font.md` — honor `command_palette_font` / `command_palette_font_size` (**user-ok**)
- `docs/plans/062-window-decorations.md` — honor `window_decorations` (native TITLE vs INTEGRATED_BUTTONS; **user-ok**)
- `docs/adr/` — accepted decisions (0003 = GPUI-owned FreeType)
- `docs/decisions/` — 016 paint quality; 017 line sprites; 018 ConPTY junk later; 019 gpui-fps HUD; 020 lua config first slice; 021 mouse selection + copy/paste; 022 click ≠ selection + term focus; 023 box-draw + cell clip; 024 4K line-sprite dest; 025 120dpi clip+dest; 026 monitor-move hang backlog; 027 new-tab shell menu; 028 icon assets; 029 cursor cell span; 030 integer cell grid; 031 cursor at cursor.x; 032 tab close/exit keys; 033 last-tab `exit` quits; 034 lua second slice; 035 lua-config.json matrix; 036 palette+launcher matrices; 037 dialog cancel restores term focus; 038 mount Root dialog layer; 039 skip close confirm for shells; 040 tab/shell splits; 041 inactive pane HSB; 042 palette selection contrast; 043 palette arrow keys keep selection; 044 palette core/tab chrome; 045 palette arrow keys scroll the list; 046 palette pane ops; 047 palette window chrome; 048 palette AdjustPaneSize; 049 palette catalog complete; 050 overlay parity; 051 cascade New Window; 052 close this HWND + search copy; 053 mux WSL/exec domains; 054 Tab sends PTY completion; 055 mux workspaces; 056 search overlay no PTY shrink; 057 tab title from dropdown; 058 hyperlink hover underline; 059 plain click opens link; 060 honor disable_default_mouse_bindings; 061 command_palette_font; 062 window_decorations
